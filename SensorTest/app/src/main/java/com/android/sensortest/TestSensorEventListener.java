...
public class TestSensorEventListener implements SensorEventListener2 {
    public static final String TAG = "TestSensorEventListener";
    ...
    private long mOnSensorChangedTime;
    ...
    public void onSensorChanged(SensorEvent event) {
        ...
        checkHandler();
        Log.d(TAG, "onSensorChanged event:"+Arrays.toString(event.values)+",timestampNs:"+timestampNs);
    ...
    public void onFlushCompleted(Sensor sensor) {
        checkHandler();
        long timestampNs = SystemClock.elapsedRealtimeNanos();
        Log.d(TAG, "onFlushCompleted sensor:"+sensor+",timestampNs:"+timestampNs);
        synchronized (mTimeStampFlushCompleteEvents) {
            mTimeStampFlushCompleteEvents.add(timestampNs);
        }
        Log.d(TAG, "onFlushCompleted mTimeStampFlushCompleteEvents:"+mTimeStampFlushCompleteEvents+".");
        synchronized (mFlushLatches) {
    ...
    /**
     * @return A list of {@link mTimeStampFlushCompleteEvents}s collected by the listener.
     */
    public List<Long> getTimeStampFlushCompleteEvents() {
        synchronized (mTimeStampFlushCompleteEvents){
            List<Long> collectedTimeStampFlushCompleteEventsList = (List)mTimeStampFlushCompleteEvents.clone();
            Log.d(TAG, "getTimeStampFlushCompleteEvents collectedTimeStampFlushCompleteEventsList:"+collectedTimeStampFlushCompleteEventsList+".");
            return Collections.unmodifiableList(collectedTimeStampFlushCompleteEventsList);
        }
    }
    /**
    ...
    public void logCollectedEventsToFile(String fileName, long deviceWakeUpTimeMs)
        ...
        //File eventsDirectory = TestSensorManager.getSensorTestDataDirectory("events/");
    ...
    public void waitForFlushComplete(CountDownLatch latch,
        ...
        try {
            String message = TestSensorManager.formatAssertionMessage(
                    "WaitForFlush",
                    mEnvironment,
                    "timeout=%dus",
                    FLUSH_TIMEOUT_US);
            if(!latch.await(FLUSH_TIMEOUT_US, TimeUnit.MICROSECONDS)) {
                Log.d(TAG, "Assert.fail message:"+message);
            }
